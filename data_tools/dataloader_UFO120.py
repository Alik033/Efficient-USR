...
class UFO120_Dataset(data.Dataset):
    ...
    def __preprocess__(self):
        ...
        ufo120hr_path  = os.path.join(self.ufo120_root,"hr")
        ufo120lr_path  = os.path.join(self.ufo120_root,"lr_%dx"%self.i_s)

        print("processing UFO120 images...")
        temp_path   = os.path.join(ufo120hr_path,'*.%s'%(self.subffix))
        images      = glob.glob(temp_path)
        for item in images:
            file_name   = os.path.basename(item)
            lr_name     = os.path.join(ufo120lr_path,file_name)
            data_path_list.append([item,lr_name])
        
        random.shuffle(data_path_list)
        # self.dataset = images
        print('Finished preprocessing the UFO120 dataset, total image number: %d...'%len(data_path_list))
...
def GetLoader(  dataset_roots,
    ...
    content_dataset = UFO120_Dataset(
                        ufo120_root,
                        c_transforms,
                        lr_patch_size,
                        image_scale,
